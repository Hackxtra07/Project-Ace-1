...
def load_projects():
    ...
    project_tree.tag_configure("High", background="lightcoral")
    project_tree.tag_configure("Medium", background="lightyellow")
    project_tree.tag_configure("Low", background="lightgreen")
...
def load_tasks():
    ...
    task_tree.tag_configure("Pending", background="lightyellow")
    task_tree.tag_configure("In Progress", background="lightblue")
    task_tree.tag_configure("Completed", background="lightgreen")
...
def load_targets():
    ...
    target_tree.tag_configure("Pending", background="lightyellow")
    target_tree.tag_configure("Completed", background="lightgreen")
...
def update_dashboard():
    ...
    for p in projects:
        ...
        tk.Label(dashboard_tab, text=f"Project: {p[1]} - {progress}% Completed", font=("Helvetica", 12, "bold")).grid(
            row=row, column=0, sticky='w', padx=10, pady=5)
        pb = ttk.Progressbar(dashboard_tab, length=400, maximum=100, value=progress)
        pb.grid(row=row, column=1, padx=10, pady=5)
        row += 1
...
def open_project_folder():
    selected = project_tree.selection()
    if not selected:
        show_message("Select a project first!")
        return
    project_name = project_tree.item(selected[0])['values'][1]
    folder_path = filedialog.askdirectory(title=f"Select folder for project '{project_name}'")
    if not folder_path:
        return

    file_win = tk.Toplevel(root)
    file_win.title(f"Files in {project_name}")
    file_win.geometry("800x500")

    tk.Label(file_win, text=f"Project: {project_name}", font=("Helvetica", 14, "bold")).pack(pady=5)

    file_listbox = tk.Listbox(file_win, width=80)
    file_listbox.pack(side='left', fill='both', expand=True, padx=5, pady=5)

    scrollbar = tk.Scrollbar(file_win)
    ...
    def edit_file():
        ...
        if not sel:
            show_message("Select a file to edit!")
            return
        ...
        editor_win = tk.Toplevel(file_win)
        ...
        def save_file():
            ...
            with open(filepath, "w", encoding="utf-8") as f:
                f.write(new_content)
            show_message(f"Saved {filename} successfully!")
            editor_win.destroy()

        tk.Button(editor_win, text="Save", bg="#5dade2", fg="white", command=save_file).pack(pady=5)

    tk.Button(file_win, text="Edit Selected File", bg="#58d68d", fg="white", command=edit_file).pack(pady=5)


# ---------------- GUI ELEMENTS ----------------

# Projects Tab
proj_frame = tk.Frame(project_tab)
proj_frame.pack(side='top', fill='x', padx=10, pady=10)
tk.Label(proj_frame, text="Name").grid(row=0, column=0)
tk.Entry(proj_frame, textvariable=project_name_var).grid(row=0, column=1)
tk.Label(proj_frame, text="Start Date").grid(row=0, column=2)
tk.Entry(proj_frame, textvariable=project_start_var).grid(row=0, column=3)
tk.Label(proj_frame, text="End Date").grid(row=0, column=4)
tk.Entry(proj_frame, textvariable=project_end_var).grid(row=0, column=5)
tk.Label(proj_frame, text="Priority").grid(row=0, column=6)
ttk.Combobox(proj_frame, textvariable=project_priority_var, values=["High", "Medium", "Low"]).grid(row=0, column=7)
tk.Button(proj_frame, text="Add", command=add_project).grid(row=0, column=8, padx=5)
tk.Button(proj_frame, text="Update", command=update_project).grid(row=0, column=9, padx=5)
tk.Button(proj_frame, text="Delete", command=delete_project).grid(row=0, column=10, padx=5)
tk.Button(proj_frame, text="Open Folder", bg="#af7ac5", fg="white", command=open_project_folder).grid(row=0, column=11,
                                                                                                      padx=5)
...
# Members Tab
mem_frame = tk.Frame(member_tab)
mem_frame.pack(side='top', fill='x', padx=10, pady=10)
tk.Label(mem_frame, text="Name").grid(row=0, column=0)
tk.Entry(mem_frame, textvariable=member_name_var).grid(row=0, column=1)
tk.Label(mem_frame, text="Role").grid(row=0, column=2)
tk.Entry(mem_frame, textvariable=member_role_var).grid(row=0, column=3)
tk.Button(mem_frame, text="Add", command=add_member).grid(row=0, column=4, padx=5)
tk.Button(mem_frame, text="Update", command=update_member).grid(row=0, column=5, padx=5)
tk.Button(mem_frame, text="Delete", command=delete_member).grid(row=0, column=6, padx=5)
...
# Tasks Tab
task_frame = tk.Frame(task_tab)
task_frame.pack(side='top', fill='x', padx=10, pady=10)
tk.Label(task_frame, text="Name").grid(row=0, column=0)
tk.Entry(task_frame, textvariable=task_name_var).grid(row=0, column=1)
tk.Label(task_frame, text="Project ID").grid(row=0, column=2)
tk.Entry(task_frame, textvariable=task_project_var).grid(row=0, column=3)
tk.Label(task_frame, text="Assigned To").grid(row=0, column=4)
tk.Entry(task_frame, textvariable=task_member_var).grid(row=0, column=5)
tk.Label(task_frame, text="Status").grid(row=0, column=6)
ttk.Combobox(task_frame, textvariable=task_status_var, values=["Pending", "In Progress", "Completed"]).grid(row=0,
                                                                                                            column=7)
tk.Label(task_frame, text="Due Date").grid(row=0, column=8)
tk.Entry(task_frame, textvariable=task_due_var).grid(row=0, column=9)
tk.Label(task_frame, text="Comments").grid(row=0, column=10)
tk.Entry(task_frame, textvariable=task_comments_var).grid(row=0, column=11)
tk.Button(task_frame, text="Add", command=add_task).grid(row=0, column=12, padx=5)
tk.Button(task_frame, text="Update", command=update_task).grid(row=0, column=13, padx=5)
tk.Button(task_frame, text="Delete", command=delete_task).grid(row=0, column=14, padx=5)

task_tree = ttk.Treeview(task_tab, columns=("ID", "Name", "ProjectID", "AssignedTo", "Status", "DueDate", "Comments"),
                         show="headings")
...
# Targets Tab
target_frame = tk.Frame(target_tab)
target_frame.pack(side='top', fill='x', padx=10, pady=10)
tk.Label(target_frame, text="Name").grid(row=0, column=0)
tk.Entry(target_frame, textvariable=target_name_var).grid(row=0, column=1)
tk.Label(target_frame, text="Project ID").grid(row=0, column=2)
tk.Entry(target_frame, textvariable=target_project_var).grid(row=0, column=3)
tk.Label(target_frame, text="Status").grid(row=0, column=4)
ttk.Combobox(target_frame, textvariable=target_status_var, values=["Pending", "Completed"]).grid(row=0, column=5)
tk.Label(target_frame, text="Due Date").grid(row=0, column=6)
tk.Entry(target_frame, textvariable=target_due_var).grid(row=0, column=7)
tk.Label(target_frame, text="Description").grid(row=0, column=8)
tk.Entry(target_frame, textvariable=target_desc_var).grid(row=0, column=9)
tk.Button(target_frame, text="Add", command=add_target).grid(row=0, column=10, padx=5)
tk.Button(target_frame, text="Update", command=update_target).grid(row=0, column=11, padx=5)
tk.Button(target_frame, text="Delete", command=delete_target).grid(row=0, column=12, padx=5)

target_tree = ttk.Treeview(target_tab, columns=("ID", "Name", "ProjectID", "DueDate", "Status", "Description"),
                           show="headings")
